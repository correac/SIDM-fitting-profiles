import numpy as np
from pylab import *
from functions import cross_section_weighted, cross_section
...
def read_data(name, num_halos):
    ...
    for i in range(0, num_halos):
        ...
        w0 = np.median(10**data[:, 4])
        v0 = np.median(10**data[:, 1])
        sigma0 = np.median(10**data[:, 3])
        low_sigma0 = sigma0-np.percentile(10**data[:, 3], 16)
        up_sigma0 = np.percentile(10**data[:, 3], 84)-sigma0

        sigma0 = cross_section_weighted(v0, sigma0, w0)
        ...
        sigma_vpair[:, i] = np.array([sigma0, sigma0 + up_sigma0, sigma0 - low_sigma0])
...
def plot_outputs():
    ...
    ax.tick_params(direction='in', axis='both', which='both', pad=4.5)
    plt.savefig('cross_section_sigma01.png', dpi=200)
