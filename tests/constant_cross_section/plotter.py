...
from functions import rho_joint_profiles, velocity_dispersion, find_rho0
from functions import calc_rs, calc_rhos
...
def model_params(N0, v0, ns0, sigma0,  log10M200, c200):
    Msun_in_cgs = 1.98848e33
    kpc_in_cgs = 3.08567758e21

    t_age = 7.5  # Gyr - assuming constant halo age
    rho0 = find_rho0(N0, t_age, v0, ns0, sigma0)
    ...
    sol = fsolve(find_r1, 20, args=(rho0_cgs, v0, ns0, t_age_cgs, sigma0))
    r1 = sol[0] * r0  # kpc
...
def plot_solution(xdata, ydata, yerrdata, soln, output_name, output_file):
    ...
    # Extract solution:
    N0, v0, ns0, sigma0, logM200, logc200 = soln
    N0 = 10 ** N0
    v0 = 10 ** v0
    sigma0 = 10 ** sigma0
    c200 = 10** logc200
    ...
    r1, rho1, r0, rho0, rs, rhos = model_params(N0, v0, ns0, sigma0, logM200, c200)
...
def plot_isothermal_fit(xdata, ydata, yerrdata, N0, v0, ns0, sigma0, output_name, output_veldisp):

    t_age = 7.5  # Gyr - assuming constant halo age
    rho0 = find_rho0(N0, t_age, v0, ns0, sigma0) # Msun / kpc^3
...
def output_best_fit_params(sol_median, sol_upper, sol_lower, output_file):
    ...
    N0, v0, ns0, med_sigma0, log10M200, c200 = sol_median
    med_floats = np.array([c200, log10M200,N0, v0, ns0, med_sigma0], dtype="object")

    N0, v0, ns0, sigma0, log10M200, c200 = sol_upper
    upp_floats = np.array([c200, log10M200,N0, v0, ns0, sigma0], dtype="object")

    N0, v0, ns0, sigma0, log10M200, c200 = sol_lower
    low_floats = np.array([c200, log10M200,N0, v0, ns0, sigma0], dtype="object")
    ...
    header += "velocity (v0) [km/s], cross section (sigmaT, SigmaVrms) [cm^2/g]."
    names = np.array(['c200', 'M200','N0', 'v0', 'ns0', 'sigma0'])
