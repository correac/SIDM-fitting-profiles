...
def calc_rhos(c200):
    ...
    rhos = (200./3.) * rhocrit * c200**3 / Yc200
    return rhos
...
def find_r1(x, rho0, v0, ns0, t_age, sigma0, w0, op_sigma_model):
    """
    ...
    if op_sigma_model == "velocity-dependent":

        sigma_v_weight = sigma_vel_weight(x, ns0, v0, sigma0, w0) # km/s cm^2/g
    else:
        sigma_v_weight = 4. * v0 * sigma0 / np.sqrt(np.pi)
...
def find_rho0(N0, t_age, v0, ns0, sigma0, w0, op_sigma_model):
...
def find_nfw(x, r1, rho1, M1):
    ...
    rs = calc_rs(10**x[0], 10**x[1])
    rhoNFW = rho_nfw(r1 / rs)
    if rhoNFW <= 0: rhoNFW = 1.

    MNFW = M_nfw(r1, 10**x[0], 10**x[1])
    if MNFW <= 0: MNFW = 1.
...
def rho_joint_profiles(r, r1, r0, rho0, ns0, log10M200, c200):
...
def calculate_log_N0(rho0, v0, ns0, sigma0, w0, op_sigma_model):
    """
...
def fit_modified_isothermal_model(xdata, a, b, n):
    xrange = np.arange(-5, 5, 0.01)
    xrange = 10**xrange
    xrange = xrange / a
    y0 = [0, 0]
...
def find_nfw_params(N0, v0, ns0, sigma0, w0, log10M200, log10c200, op_sigma):

    Msun_in_cgs = 1.98848e33
    kpc_in_cgs = 3.08567758e21

    t_age = 7.5 # Gyr - assuming constant halo age
    rho0 = find_rho0(N0, t_age, v0, ns0, sigma0, w0, op_sigma)
    t_age_cgs = t_age * 1e9 * 365.24 * 24 * 3600  # sec
    ...
    sol = fsolve(find_r1, 20, args=(rho0_cgs, v0, ns0, t_age_cgs, sigma0, w0, op_sigma))
    r1 = sol[0] * r0 # kpc

    M1 = M_isothermal(r1, r0, rho0, ns0) # Msun
    rho1 = rho0 * rho_isothermal( r1/r0, ns0) # Msun /kpc^3

    sol = root(find_nfw, [log10M200, log10c200], args=(r1, np.log10(rho1), np.log10(M1)), method='hybr', tol=1e-4)
